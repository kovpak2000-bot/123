...
/*	1.1 (18.02.2024) by mx?!:
	* Попытка поправить ошибку "[MySQL] Thread worker was unable to start."
		* Добавлена логика g_bPluginEnded
		* Регистрация событий перенесена в событие перехода системы в состояние "готов" (csxsql_initialized)
	* Удалена поддержка AMXX < 183
	* Фикс изначального (при загрузке плагина) состояния квара pause_stats
	* Добавлена проверка готовности системы для srvcmd "csstats_sql_reset"
	* Ошибки теперь пишутся в отдельный лог "csstatsx_sql_errors.log", а не в лог amxx
*/

#include <amxmodx>
#include <sqlx>
#include <gather>
#include <bot_features>

//#define DEBUG "csstatsx_sql_debug.log"

new const ERR_LOGFILE[] = "csstatsx_sql_errors.log"
...
#define VERSION "1.1" // based on 0.7.4+2
...
new skill_que[QUERY_LENGTH * 3],skill_que_len
new statsnum
...
public plugin_precache() {
	...
	bind_pcvar_num(cvar[CVAR_PAUSE], pause_stats)
...
public plugin_init()
{
	...
	g_fwdClientLoaded = CreateMultiForward("client_loaded", ET_IGNORE, FP_CELL)
	...
	//set_task(2.5, "task_InitSql")
	set_task(4.0, "task_InitSql")
}

public task_InitSql() //OnConfigsExecuted()
{
	...
	if(get_pcvar_num(cvar[CVAR_SQL_CREATE_DB]))
	{
		...
		DB_AddInitSeq()
		ThreadQuery(sql,"SQL_Handler",query,sql_data,sizeof sql_data)
...
{
	if(!is_ready) {
		server_print("System not initialized yet!")
...
public BotFeatures_KilledPost(pVictim, pKiller) {
	if(!is_user_connected(pKiller) || get_member(pVictim, m_bKilledByBomb)) {
		Stats_SaveKill(0,pVictim,0,0)
		return
	}

	static weapon_id,last_hit//,attacker
	//attacker = get_user_attacker(pVictim,weapon_id,last_hit)
	get_user_attacker(pVictim,weapon_id,last_hit)

	Stats_SaveKill(pKiller,pVictim,weapon_id,last_hit)
}
...
DB_SavePlayerData(id,bool:reload = false)
{
	...
	switch(player_data[id][PLAYER_LOADSTATE])
	{
		case LOAD_OK: // обновление данных
		{
		...
		#if defined DEBUG
			new Float:skill = player_data[id][PLAYER_SKILL]
			new Float:last_skill = player_data[id][PLAYER_SKILLLAST]
		#endif
			...
			#if defined DEBUG
				if(g_bSkillChanged) {
					log_to_file(DEBUG, "player %n, skill %f last_skill %f (diff %f)", id, skill, last_skill, diffskill)
					log_to_file(DEBUG, "new skill will be %f", last_skill + diffskill)
				}
			#endif
			...
			#if defined DEBUG
				log_to_file(DEBUG, "[1] player %n", id)
			#endif
			}
			...
			{
			#if defined DEBUG
				log_to_file(DEBUG, "[2] player %n", id)
			#endif
...
#if defined DEBUG
	log_to_file(DEBUG, "[3] player %n", id)
#endif

	if(query[0])
	{
	#if defined DEBUG
		log_to_file(DEBUG, "[4] player %n", id)
	#endif

		if(weapon_stats_enabled)
		{
		#if defined DEBUG
			log_to_file(DEBUG, "[5] player %n", id)
		#endif
			DB_SavePlayerWstats(id)
		}

	#if defined DEBUG
		log_to_file(DEBUG, "[6] player %n (sql_data[0] %i, should be %i)", id, sql_data[0], SQL_UPDATE)
	#endif
		...
		switch(sql_data[0])
		{
			...
			{
			#if defined DEBUG
				log_to_file(DEBUG, "[7] player %n", id)
			#endif
	...
	#if defined DEBUG
		log_to_file(DEBUG, "[8] player %n", id)
	#endif
		ThreadQuery(sql,"SQL_Handler",query,sql_data,sizeof sql_data)
	}
...
DB_AddQuery(query[],len)
{
	...
	{
	#if defined DEBUG
		log_to_file(DEBUG, "Force flush from DB_AddQuery()")
	#endif
		DB_FlushQuery()
	}

#if defined DEBUG
	log_to_file(DEBUG, "DB_AddQuery flush_que(len %i) before add: %s", flush_que_len, flush_que)
#endif
...
#if defined DEBUG
	log_to_file(DEBUG, "DB_AddQuery flush_que(len %i) after add: %s", flush_que_len, flush_que)
#endif
...
public DB_FlushQuery()
{
	...
	#if defined DEBUG
		log_to_file(DEBUG, "DB_FlushQuery: %s", flush_que)
	#endif
		ThreadQuery(sql,"SQL_Handler",flush_que,sql_data,sizeof sql_data)
...
public DB_GetPlayerRanks()
{
	...
	if(letsgo)
	{
		...
		ThreadQuery(sql,"SQL_Handler",query,data,sizeof data)
	}
}
...
public SQL_Handler(failstate,Handle:sqlQue,err[],errNum,data[],dataSize){
	...
	switch(failstate)
	{
		...
		{
			log_to_file(ERR_LOGFILE, "SQL connection failed")
			log_to_file(ERR_LOGFILE, "[ %d ] %s",errNum,err)
		...
		case TQUERY_QUERY_FAILED:  // ошибка SQL запроса
		{
			...
			log_to_file(ERR_LOGFILE, "SQL query failed")
			log_to_file(ERR_LOGFILE, "[ %d ] %s",errNum,err)
			log_to_file(ERR_LOGFILE, "[ SQL ] %s",lastQue)
	...
	switch(data[0])
	{
		...
		case SQL_LOAD: // загрзука статистики игрока
		{
			...
			if(SQL_NumResults(sqlQue)) // считываем статистику
			{
				...
				// скилл
				SQL_ReadResult(sqlQue,ROW_SKILL,player_data[id][PLAYER_SKILL]) // TODO не в этом дело?
				player_data[id][PLAYER_SKILLLAST] = _:player_data[id][PLAYER_SKILL]
				...
				ExecuteForward(g_fwdClientLoaded, _, id);
			}
		...
		case SQL_GETSESTATS:
		{
			...
			{
				log_to_file(ERR_LOGFILE, "get_sestats_thread_sql callback function failed")
			}
		}
		case SQL_AUTOCLEAR:
		{
...
public plugin_natives()
{
	...
	register_native("set_user_skill","native_set_user_skill")
...
public native_get_sestats_thread_sql(plugin_id,params)
{
	...
	ThreadQuery(sql,"SQL_Handler",query,sql_data,3 + data_size)
...
public native_set_user_skill(plugin_id,params)
{
	new player = get_param(1)
	new Float:skill = get_param_f(3)

	if(player) {
	#if defined DEBUG
		g_bSkillChanged = true
	#endif
		player_data[player][PLAYER_SKILL] = skill
		return 1
	}

	static authid[64]
	get_string(2, authid, charsmax(authid))

	static query[QUERY_LENGTH]

	new len = formatex( query, charsmax(query), "UPDATE `%s` SET `%s` = %.2f WHERE `%s` = '%s'",
		tbl_name, row_names[ROW_SKILL], skill, row_names[ROW_STEAMID], authid );

	DB_AddSkillQuery(query,len)
...
DB_AddSkillQuery(query[],len)
{
	if((skill_que_len + len + 1) > charsmax(skill_que))
	{
		DB_FlushSkillQuery()
	}

	skill_que_len += formatex(
		skill_que[skill_que_len],
		charsmax(skill_que) - skill_que_len,
		"%s%s",skill_que_len ? ";" : "",
		query
	)

	// задание на сброс накопленных запросов
	remove_task(task_skill)
	set_task(0.1,"DB_FlushSkillQuery",task_skill)
}

public DB_FlushSkillQuery()
{
	if(skill_que_len)
	{
	#if defined DEBUG
		log_to_file(DEBUG, "skill_que_len %i, skill_que: %s", skill_que_len, skill_que)
	#endif
		new sql_data[1] = SQL_UPDATE_SKILL
		ThreadQuery(sql,"SQL_Handler",skill_que,sql_data,sizeof sql_data)

		skill_que_len = 0
	}
}

ThreadQuery(Handle:sql, const handler[], const sql_query[], const sql_data[], max_len) {
#if defined DEBUG
	log_to_file(DEBUG, "query: %s", sql_query)
#endif
	SQL_ThreadQuery(sql, handler, sql_query, sql_data, max_len)
}
...
public native_get_stats(plugin_id,params)
{
	...
	*/
	...
	if(!SQL_Execute(sqlQue))
	{
		...
		log_to_file(ERR_LOGFILE, "SQL query failed")
		log_to_file(ERR_LOGFILE, "[ %d ] %s",errNum,err)
		log_to_file(ERR_LOGFILE, "[ SQL ] %s",query)
